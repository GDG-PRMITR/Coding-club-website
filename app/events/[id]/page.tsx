...
export async function generateMetadata({ params }: EventDetailProps): Promise<Metadata> {
  ...
  return {
    title: `${event.title} | Prof. Ram Meghe Institute of Technology and Research`,
    description: `Official event at PRMITR: ${event.description}`,
    keywords,
    alternates: {
...
export default async function EventDetailPage({ params }: EventDetailProps) {
  ...
  // JSON-LD for Search Engine "Event" snippets
  const jsonLd = {
    ...
    organizer: {
      "@type": "Organization",
      name: `${event.club.toUpperCase()} PRMITR`,
      ...
      <div className="mx-auto max-w-5xl space-y-8">
        ...
        <div className="mt-16 border-t border-slate-200 pt-8 text-center text-slate-500 dark:border-slate-800">
          <p className="mx-auto max-w-4xl text-sm italic">
            This event is organized by the <strong>Coding Club</strong> and its partner communities at
            <strong> Prof. Ram Meghe Institute of Technology and Research (PRMITR)</strong>, Badnera.
            Stay tuned for more high-impact workshops and community events in Amravati.
          </p>
        </div>
      </div>
    </>
